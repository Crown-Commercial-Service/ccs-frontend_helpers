...
require 'ccs/components/ccs/footer/link'

RSpec.describe CCS::Components::CCS::Footer::Link do
  ...
  describe '.render' do
    let(:ccs_footer_navigation_link) { described_class.new(text: text, href: href, li_class: li_class, context: view_context, **options) }
    let(:result) { ccs_footer_navigation_link.render }

    let(:text) { 'Go Beyond' }
    let(:href) { '/go-beyond' }
    let(:li_class) { 'ccs-footer__navigation-item' }

    let(:options) { {} }

    let(:default_html) { '<li class="ccs-footer__navigation-item"><a class="ccs-footer__link" href="/go-beyond">Go Beyond</a></li>' }
    ...
    context 'when no options are sent' do
      let(:ccs_footer_navigation_link) { described_class.new(text: text, href: href, li_class: li_class, context: view_context) }
    ...
    context 'when additional classes are passed' do
      ...
      it 'does not have the custom class' do
        expect(footer_navigation_item_link[:class]).to eq 'ccs-footer__link'
      end
    end
