...
require 'ccs/components/govuk/notification_banner'

RSpec.describe CCS::Components::GovUK::NotificationBanner do
  include_context 'and I have a view context'

  let(:notification_banner_element) { Capybara::Node::Simple.new(result).find('.govuk-notification-banner') }

  describe '.render' do
    let(:govuk_notification_banner) { described_class.new(text: text, success_banner: success_banner, context: view_context, **options) }
    let(:result) { govuk_notification_banner.render }
    ...
    context 'when the no options are sent' do
      let(:govuk_notification_banner) { described_class.new(text: text, success_banner: success_banner, context: view_context) }
    ...
    context 'when no parameters are passed' do
      let(:govuk_notification_banner) { described_class.new(context: view_context) }
    ...
    context 'when custom role is passed' do
      let(:options) { { attributes: { role: 'banner' } } }
    ...
    context 'when I pass in content' do
      let(:result) do
        govuk_notification_banner.render do
          view_context.concat(tag.h3('You have defeated Mobius', class: 'govuk-notification-banner__heading'))
          view_context.concat(tag.p(class: 'govuk-body') do
            concat('Contact ')
