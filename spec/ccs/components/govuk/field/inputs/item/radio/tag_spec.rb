...
require 'ccs/components/govuk/field/inputs/item/radio/tag'

RSpec.describe CCS::Components::GovUK::Field::Inputs::Item::Radio::Tag do
  ...
  describe '.render' do
    let(:govuk_radio_item) { described_class.new(attribute: attribute, value: value, label: label, hint: hint, conditional: conditional, context: view_context, **options) }
    ...
    context 'when some options are not passed' do
      let(:govuk_radio_item) { described_class.new(attribute: attribute, value: value, label: label, context: view_context) }
    ...
    context 'when the value us an iregular value' do
      ...
      it 'is able to format an ID' do
        expect(radio_item_input_element[:id]).to eq('ouroboros_Zeke_Von_Gembu__Bringer_of_Chaos')
        expect(radio_item_label_element[:for]).to eq('ouroboros_Zeke_Von_Gembu__Bringer_of_Chaos')
      end
    end
