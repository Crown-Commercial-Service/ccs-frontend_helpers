...
require 'ccs/components/govuk/step_by_step_navigation'

RSpec.describe CCS::Components::GovUK::StepByStepNavigation do
  include_context 'and I have a view context'

  let(:step_by_step_navigation_element) { Capybara::Node::Simple.new(result).find('.gem-c-step-nav') }

  describe '.render' do
    let(:govuk_step_by_step_navigation) { described_class.new(step_by_step_navigation_sections: step_by_step_sections, context: view_context, **options) }
    let(:result) { govuk_step_by_step_navigation.render }
    ...
    context 'when some options are not passed' do
      let(:govuk_step_by_step_navigation) { described_class.new(step_by_step_navigation_sections: step_by_step_sections, context: view_context) }
