...
RSpec.describe CCS::FrontendHelpers::GovUKFrontend do
  ...
  describe 'all GOV.UK Frontend components are created' do
    Dir["#{FixturesLoader::FIXTURE_PATHS[:govuk_frontend]}/**/macro.njk"].each do |helper_module_file|
      helper_module = "CCS::FrontendHelpers::GovUKFrontend::#{helper_module_file[(FixturesLoader::FIXTURE_PATHS[:govuk_frontend].length + 1)...-10].gsub('-', '_').camelize}"
