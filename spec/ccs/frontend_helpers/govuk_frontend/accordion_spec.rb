...
RSpec.describe CCS::FrontendHelpers::GovUKFrontend::Accordion, type: :helper do
  ...
  describe '.govuk_accordion' do
    ...
    context 'when one section is expanded' do
      before { accordion_items[1][:expanded] = true }

      it 'has the expanded class for that section' do
        expect(accordion_sections[0][:class]).to eq('govuk-accordion__section')
        expect(accordion_sections[1][:class]).to eq('govuk-accordion__section govuk-accordion__section--expanded')
        expect(accordion_sections[2][:class]).to eq('govuk-accordion__section')
      end
    end
